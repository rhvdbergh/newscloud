...
function sanitizeStr(str, removeWords) {
  ...
  newStr = sw
    .removeStopwords(newStr.split(' '), [...removeWords, ...stopwordList])
    .join(' ');
...
router.get('/:searchTerm', function (req, res, next) {
  ...
  if (searchTerm !== 'favicon.ico') {
    // if GET /favicon.ico, just ignore and skip forward
    SearchTerm.findOne({ searchTerm: searchTerm }, null, {}, (err, term) => {
      ...
      if (term) {
        // there is already an instance of the search term in the db
        let date = new Date();
        term.count++;
        term.lastUpdated = date;
        term.save();
      } else {
        // create a new entry in the db for this search term
  ...
  T.get(
    ...
    function (err, data, response) {
      ...
      request(options, (err, response, body) => {
        if (err) {
          err.message =
            'Error retrieving news headlines and summaries: ' + err.message;
          console.log(err.message);
          next(err);
        }
        ...
        if (data.status === 'error' && data.code === 'rateLimited') {
          let err = new Error(
            'NewsCloud has exceed its rate limit for the News API. Please try again later.'
          );
          let newsStr =
            'Unfortunately, no news today. Unfortunately, too, no news is bad news in this case ... the News API rate limit has been exceeded for this web app.';
          next(err);
        }
        let newsStr = '';
...
router.get('/random/search', (req, res, next) => {
  ...
  request(
    {
      url: `https://api.datamuse.com/words?sp=${rndLtrA}${wildcards}${rndLtrB}&max=1`,
    },
    (err, response, body) => {
      if (err) {
